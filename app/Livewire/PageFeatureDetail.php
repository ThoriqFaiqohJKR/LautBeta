...
class PageFeatureDetail extends Component
{
    public array $item = [];

    public function mount(): void
    {
        $id      = (int) request()->route('id');   // ambil {id} dari URL
        $urlSlug = request()->route('slug');       // slug opsional
        $locale  = app()->getLocale();             // sudah diset oleh middleware
    ...
    private function imageUrl(?string $path): string
    {
        ...
        if (str_starts_with($path, 'storage/')) return asset($path); // public/storage
    ...
    {
        return view('livewire.page-feature-detail');
    }
}
