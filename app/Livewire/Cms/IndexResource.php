...
class IndexResource extends Component
{
    ...
    public function render()
    {
        ...
        $reportQ = DB::table('report')
            ...
            ->when($this->q !== '', function ($q) use ($title, $desc) {
                $s = '%' . $this->q . '%';
                $q->where(fn($w) => $w->where($title, 'like', $s)->orWhere($desc, 'like', $s)->orWhere('slug', 'like', $s));
            });
        ...
        $databaseQ = DB::table('database')
            ...
            ->when($this->databaseSearch !== '', function ($q) use ($title) {
                $s = '%' . $this->databaseSearch . '%';
        ...
        $galleryQ = DB::table('gallery')
            ...
            ->when($this->galleryQ !== '', function ($q) use ($title) {
                $s = '%' . $this->galleryQ . '%';
                $q->where(fn($w) => $w->where($title, 'like', $s)->orWhere('slug', 'like', $s)->orWhere('type', 'like', $s));
            });

        $reports   = $this->applySort($reportQ, $this->sort)->get();
        $databases = $this->applySort($databaseQ, $this->databaseSort)->get();
        $galleries = $this->applySort($galleryQ, $this->gallerySort)->get();
